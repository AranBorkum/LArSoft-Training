...
private:
  ...
  void  FillTruth     (const art::FindManyP<simb::MCParticle> Assn,
		       ...
		       const PType type);
  void  SaveIDEs      (art::Event const & e);
...
void FullGeoAna::analyze(art::Event const & evt) {
  ...
  */
  ...
  for (size_t it=0; it<particleHandle->size(); it++) {
    ...
    xyz1[1] = part.Vy();
    xyz1[2] = part.Vz();
    geoManager->SetCurrentPoint(xyz1);
    TGeoNode* node1 = geoManager->FindNode();
    std::string volmaterial1 = node1->GetMedium()->GetMaterial()->GetName();
    True_Bck_StartMaterial.push_back(volmaterial1);
    delete xyz1;
    xyz1 = nullptr;
    ...
    xyz[1] = part.EndY();
    xyz[2] = part.EndZ();
    geoManager->SetCurrentPoint(xyz);
    TGeoNode* node = geoManager->FindNode();
    std::string volmaterial;
    if (node) volmaterial = node->GetMedium()->GetMaterial()->GetName();
    else      volmaterial = "UNKNOWN";
    True_Bck_EndMaterial.push_back(volmaterial);
    delete xyz;
    xyz = nullptr;
  ...
  */                                                  
  ...
  if (fSaveTPC) {
    ...
    if (evt.getByLabel(fHitLabel     , reco_hits         ) &&
      ...
      // --- Count the number of collection and induction wire hits
      ...
      // 		<< " out of "           << rawDigitsVecHandle->size() 
      // 		<< " channels into set" << std::endl;
